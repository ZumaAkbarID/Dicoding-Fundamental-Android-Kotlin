...
class FinishEventAdapter(private val onFavClick: (EventEntity) -> Unit) :
    ListAdapter<EventEntity, FinishEventAdapter.MyViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener { view ->
            event?.let { event ->
                val action = FinishFragmentDirections.actionNavigationFinishToDetailFragment2(
                    event.id,
                    event.name,
                    false
                )
