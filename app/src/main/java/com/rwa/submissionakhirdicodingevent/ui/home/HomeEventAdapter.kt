...
class HomeEventAdapter(private val onFavClick: (EventEntity) -> Unit) :
    ListAdapter<EventEntity, HomeEventAdapter.MyViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener { view ->
            event?.let { event ->
                val action = HomeFragmentDirections.actionNavigationHomeToDetailFragment2(
                    event.id,
                    event.name
                )
